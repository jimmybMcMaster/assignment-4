...
import { type Book, type Filter } from '../adapter/assignment-3.js'
import { type ZodRouter } from 'koa-zod-router'

// Business logic 
async function listBooks(filters: Filter[] | undefined, database: BookDatabaseAccessor): Promise<Book[]> {
 ...
 const bookList = await database.books.find(query).map(document => {
   const book: Book = {
...
// Route handler
export default function booksListRoute(router: ZodRouter): void {
 router.register({
   name: 'list books',
   method: 'get',
   path: '/books',
